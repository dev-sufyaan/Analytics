...
import { TopPage } from '@aether/db/types';
import {
  PanelCard,
  TogglePillGroup,
  DataTableHeader,
...
export default function PagesBreakdownPage({
  ...
  const fetchPages = useCallback(async () => {
    ...
    try {
      const { data } = await supabase.rpc('get_top_pages', {
  ...
  const maxViews = filteredPages.length > 0 ? Math.max(...filteredPages.map((p) => Number(p.pageviews))) : 1;
    ...
    <div className="max-w-[1280px] mx-auto px-4 md:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 mb-8 border-b border-[#ebebeb] gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <ButtonOutline onClick={fetchPages} className="px-3" title="Refresh">
      ...
      <div className="space-y-6">
        ...
        <PanelCard title={`All Visited URLs (${filteredPages.length})`}>
          {filteredPages.length === 0 ? (
            <p className="text-center py-10 font-display text-[14px] text-[#71717a]">
              {loading ? 'Loading pages...' : 'No pageviews match your query.'}
            </p>
          ) : (
            <div className="border border-[#ebebeb] rounded-[4px] overflow-hidden">
              ...
              {filteredPages.map((p, idx) => (
                <DataTableRow
                  key={idx}
                  percent={(Number(p.pageviews) / maxViews) * 100}
                >
