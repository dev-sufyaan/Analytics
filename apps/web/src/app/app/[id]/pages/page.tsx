...
export default function PagesBreakdownPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return pages;
    return pages.filter((p) => p.url_path.toLowerCase().includes(q));
  }, [pages, searchQuery]);
  ...
  const maxViews = filtered.length ? Math.max(...filtered.map((p) => Number(p.pageviews))) : 1;
    ...
    <div className="max-w-[1280px] mx-auto px-4 md:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 mb-8 border-b border-[#ebebeb] gap-4">
        <div className="min-w-0">
          ...
          <p className="font-display text-[13px] text-[#71717a] mt-1">
            Every path that received a pageview in the selected window.
          </p>
        </div>
        ...
        <PanelCard>
          {loading ? (
            <div className="border border-[#ebebeb] rounded-[4px] overflow-hidden">
              <DataTableHeader
                columns={[
                  ...
                  { label: 'VISITORS', width: '140px', align: 'right' },
                ]}
              />
            ...
            <p className="text-center py-10 font-display text-[14px] text-[#71717a]">
              {searchQuery ? `No paths match “${searchQuery}”.` : 'No pageviews in this period.'}
            </p>
          ) : (
            <>
              <div className="border border-[#ebebeb] rounded-[4px] overflow-hidden">
                ...
                />
                {paginated.map((p, idx) => (
                  <DataTableRow key={`${p.url_path}-${idx}`} percent={(Number(p.pageviews) / maxViews) * 100}>
                    <span className="font-display text-[14px] text-black font-medium truncate flex-1 pr-4 min-w-0">
                      {p.url_path}
                    </span>
                    <span className="font-mono text-[13px] text-black w-[130px] text-right font-medium shrink-0">
                      {Number(p.pageviews).toLocaleString()}
                    </span>
                    <span className="font-mono text-[13px] text-[#71717a] w-[140px] text-right shrink-0">
                      {Number(p.visitors).toLocaleString()}
                    </span>
                  </DataTableRow>
                ))}
              </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between pt-4">
                  <span className="font-mono text-[11px] uppercase text-[#999999]">
