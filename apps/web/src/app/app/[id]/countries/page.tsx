...
export default function CountriesBreakdownPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchCountries = useCallback(
    async (force = false) => {
      ...
      else setCountries((peek.data.countries as TopCountry[]) ?? []);
      try {
        if (peek?.fresh && !force) return;
        const data = await loadOverview(supabase, websiteId, range, { limit: 100 });
        setCountries((data.countries as TopCountry[]) ?? []);
      } catch (e) {
    ...
    <div className="max-w-[1280px] mx-auto px-4 md:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 mb-8 border-b border-[#ebebeb] gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <ButtonOutline onClick={() => fetchCountries(true)} className="px-3" aria-label="Refresh">
      ...
      <div className="space-y-6">
        ...
        {error && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-[4px] flex items-center justify-between gap-3">
            ...
            <ButtonOutline type="button" onClick={() => fetchCountries(true)} className="h-7 px-2.5 text-[11px]">RETRY</ButtonOutline>
          </div>
        )}
