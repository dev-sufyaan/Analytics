...
import { TopCountry } from '@aether/db/types';
import {
  PanelCard,
  TogglePillGroup,
  DataTableHeader,
...
export default function CountriesBreakdownPage({
  params,
}: {
  ...
  const fetchCountries = useCallback(async () => {
    ...
    try {
      const { data } = await supabase.rpc('get_top_countries', {
  ...
  const maxVisitors = filteredCountries.length > 0 ? Math.max(...filteredCountries.map((c) => Number(c.visitors))) : 1;
    ...
    <div className="max-w-[1280px] mx-auto px-4 md:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 mb-8 border-b border-[#ebebeb] gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <ButtonOutline onClick={fetchCountries} className="px-3" title="Refresh">
      ...
      <div className="space-y-6">
        ...
        <PanelCard title={`Visitor Countries (${filteredCountries.length})`}>
          {filteredCountries.length === 0 ? (
            <p className="text-center py-10 font-display text-[14px] text-[#71717a]">
              {loading ? 'Loading country data...' : 'No geography data recorded in this period.'}
            </p>
          ) : (
            <div className="border border-[#ebebeb] rounded-[4px] overflow-hidden">
              <DataTableHeader
                columns={[
                  ...
                  { label: 'SESSIONS', width: '120px', align: 'right' },
                ]}
              />
              {filteredCountries.map((c, idx) => (
                <DataTableRow
                  key={idx}
                  percent={(Number(c.visitors) / maxVisitors) * 100}
                >
                  ...
                  <span className="font-mono text-[13px] text-[#71717a] w-[120px] text-right">
